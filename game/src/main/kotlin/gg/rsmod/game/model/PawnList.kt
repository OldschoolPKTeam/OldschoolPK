...
class PawnList<T : Pawn>(private val pawns: Array<T?>) {
    ...
    private val hashes = HashMap<Int, T>()
    ...
    fun addHash(pawn: T) {
        val key = pawn.hashCode()
        check(!hashes.containsKey(key)) { "Key [key='$key', pawn='$pawn'] is already on the map!" }
        hashes.put(key, pawn)
    }

    fun removeHash(pawn: T) {
        val key = pawn.hashCode()
        check(hashes.containsKey(key)) { "Key [key='$key', pawn='$pawn'] is not on the map!" }
        hashes.remove(key)
    }

    fun getFromHash(hash: Int): Pawn? {
