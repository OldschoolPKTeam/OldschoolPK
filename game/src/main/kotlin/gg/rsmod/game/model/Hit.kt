...
class Hit private constructor(val hitmarks: List<Hitmark>, val hitbar: Hitbar?, val clientDelay: Int, var damageDelay: Int) {
    ...
    data class Hitmark(val type: Int, var damage: Int, val attacker: Optional<WeakReference<Pawn>>) {

        /**
         * Determines whether the [type] has a tinted variant associated with it
         */
        fun isTintedHitType(): Boolean = when(this.type) {
            12, 16, 18, 20, 22, 24 -> true
            else -> false
        }
    ...
    class Builder {
         ...
         * @see Hit.hitmarks
         */
        fun addHit(damage: Int, type: Int, attacker: Optional<WeakReference<Pawn>> = Optional.empty()): Builder {
            hitmarks.add(Hitmark(type, damage, attacker))
            return this
        }
