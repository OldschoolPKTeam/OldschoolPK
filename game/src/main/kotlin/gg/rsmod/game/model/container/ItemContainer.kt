...
class ItemContainer(val definitions: DefinitionSet, val key: ContainerKey) : Iterable<Item?> {
    ...
    /**
     * Returns a list of all non-null [Item]s as a [SlotItem].
     */
    fun toSlottedList(): List<SlotItem> {
        return rawItems.mapIndexed { index, item -> if (item == null) null else SlotItem(index, item) }.filterNotNull().toList()
    }
    ...
    /**
     * Checks if the container has all the item ids of the [set]
     */
    fun containsAll(set: Set<Int>): Boolean = items.map { it?.id }.toSet().containsAll(set)
    ...
    fun add(
         ...
         */
        val stack =
            !forceNoStack && stackType != ContainerStackType.NO_STACK && (def.stackable || stackType == ContainerStackType.STACK)
    ...
    fun add(
        ...
        return add(
            item = item.id, amount = item.amount, assureFullInsertion = assureFullInsertion,
            forceNoStack = forceNoStack, beginSlot = beginSlot
        )
    }

    /**
     * Adds an [Array<Item>] to our container.
     *
     * Keep in mind, that just because you're using an [Item] object, does not
     * mean that the same [Item] instance will be added to the container. The
     * [item] is simply used as a reference for the item id and amount that should
     ...
     * account.
     */
    fun addAll(vararg items: Item, assureFullInsertion: Boolean = true, forceNoStack: Boolean = false, beginSlot: Int = -1): Array<ItemTransaction> {
        val transactions: ArrayList<ItemTransaction> = arrayListOf()
        items.forEach { transactions.add(add(it.id, it.amount, assureFullInsertion, forceNoStack, beginSlot)) }
     ...
     * ```
     */
    fun remove(item: Item, assureFullRemoval: Boolean = false, beginSlot: Int = -1): ItemTransaction {
        val slot = if (beginSlot == -1) this.indexOf(item) else beginSlot
        return remove(item.id, item.amount, assureFullRemoval, slot)
    }

    /**
     * Replaces an [Item] if it is found with a [replacementItem].
     * If the [item] is not found within the container it will return a failed
     * item transaction.
     *
     * @param item              The [Item] that is being replaced
     * @param replacementItem   The replacement [Item]
     * @param copyAttributes
     * A flag that determines whether the [item]s attributes should be copied to
     * the [replacementItem]
     */
    fun replace(item: Item, replacementItem: Item? = null, copyAttributes: Boolean = false): ItemTransaction {
        val index = this.indexOf(item)
        if (index == -1) return ItemTransaction(item.amount, 0, listOf())
        this[index] = replacementItem.apply {
            if (copyAttributes) this?.copyAttr(item)
        }
        return when {
            replacementItem == null -> ItemTransaction(1, 1, listOf())
            else -> ItemTransaction(
                replacementItem.amount, replacementItem.amount, listOf(
                    SlotItem(
                        index,
                        replacementItem
                    )
