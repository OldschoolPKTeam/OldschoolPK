    ...
    override fun calculateRoute(request: PathRequest): Route {
        ...
        while (nodes.isNotEmpty()) {
            ...
            order.sortBy {
                ...
                step.getManhattanDistance(end) + step.getManhattanDistance(head.tile)
            }
        ...
        if (tail == null && closed.isNotEmpty()) {
            val min = closed.minBy { it.tile.getIntDistance(end) }!!
            val valid = closed.filter { it.tile.getIntDistance(end) <= min.tile.getIntDistance(end) }
            if (valid.isNotEmpty()) {
                tail = valid.minBy { it.tile.getManhattanDistance(start) }
            }
        }
