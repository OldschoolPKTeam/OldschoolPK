...
class World(val gameContext: GameContext, val devContext: DevContext) {
    ...
    internal fun cycle() {
        ...
        while (groundItemIterator.hasNext()) {
            ...
            if (groundItem.isPublic() && groundItem.despawns && groundItem.currentCycle >= gameContext.gItemDespawnDelay) {
                /*
    ...
    fun queue(taskPriority: TaskPriority = TaskPriority.STRONG, logic: suspend QueueTask.(CoroutineScope) -> Unit) {
        queues.queue(this, coroutineDispatcher, taskPriority, logic)
    }
     ...
     */
    internal fun loadServices(server: Server, worldConfiguration: WorldConfiguration) {
        val stopwatch = Stopwatch.createUnstarted()
        val foundServices = worldConfiguration.get<ArrayList<Any>>("services")!!
    ...
    /**
     * A [List] of every [Player] and [Npc] in the [World]
     */
    fun pawnList(): List<Pawn> = (this.npcs.entries.toList() + this.players.entries.toList()).filterNotNull()
