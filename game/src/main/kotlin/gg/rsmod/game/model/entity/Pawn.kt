...
abstract class Pawn(val world: World) : Entity() {
    ...
    /**
     * A deque of pending [Hit]s.
     */
    private val pendingHits: Deque<Hit> = LinkedList<Hit>()
    ...
    /**
     * Whether the pawn is in a multi combat area or not
     */
    var inMulti = false
    ...
    /**
     * Update this Pawn's tile.
     */
    fun updateTile(tile: Tile) {
        this.tile = Tile(tile)
        inMulti = tile.isMulti(world)
        if (this is Player) {
            // Set multiway varbit
            // TODO move varbit stuff to core so we don't need to hack
            val def = world.definitions.get(VarbitDef::class.java, 4605)
            varps.setBit(def.varp, def.startBit, def.endBit, if (inMulti) 1 else 0)
        }
    }
    ...
    /**
     * Gets a list of [Tile]s that the [Pawn] is occupying.
     */
    fun getTiles(): List<Tile> {
        val tiles = mutableListOf<Tile>()
        for (x in tile.x until tile.x + getSize()) {
            for (z in tile.z until tile.z + getSize()) {
                tiles.add(Tile(x, z))
            }
        }
        return tiles
    ...
    fun attack(target: Pawn) {
        /*
         * Passive npcs are ones that will never attack you, even when attacked.
         */
        if (this is Npc && passive) {
            return
        }
     ...
     */
    fun timerCycle() {
        val iterator = timers.getTimers().toMutableMap().iterator()
     ...
     */
    fun hitsCycle() {
        val pendingHitCopy = pendingHits.toMutableList()
        val hitIterator = pendingHitCopy.iterator()
        iterator@ while (hitIterator.hasNext()) {
            ...
            if (hit.damageDelay-- <= 0) {
                if (!hit.cancelCondition()) {
                    if(hit.hitmarks.sumBy { it.damage } != -1) {
                    ...
                    for (hitmark in hit.hitmarks) {
                        ...
                        if (INFINITE_VARS_STORAGE.get(this, InfiniteVarsType.HP) == 0) {
                            // a check for splashing
                            if(hitmark.damage != -1) setCurrentHp(hp - hitmark.damage)
                        }
                        /*
        ...
        // Add the remaining hits from the copied list to the real list
        pendingHits.clear()
        pendingHits.addAll(pendingHitCopy)
    ...
    fun walkPath(path: Queue<Tile>, stepType: MovementQueue.StepType, detectCollision: Boolean) {
        ...
        if (stepType != MovementQueue.StepType.FORCED_WALK) {
    ...
    fun animate(id: Int, delay: Int = 0) {
        if(!this.lock.canAnimate()) return@animate
