...
import gg.rsmod.util.EntityNamer
...
class DumpEntityIdService : Service {
    ...
    private fun writeItems(definitions: DefinitionSet, namer: EntityNamer) {
        val count = definitions.getCount(ItemDef::class.java)
        val items = generateWriter("Items.kt")

        val itemIdList = (0 until count).sortedBy { if (skipItem(it)) 1 else 0 }.toList()
        for (i in itemIdList) {
    ...
    /**
     * These items are pushed to the end so they still show up but others are prioritized ahead of them.
     * They are also pushed to the bottom of the generated Items.kt file.
     */
    private fun skipItem(item: Int): Boolean {
        return when(item) {
            9771, 9772, 10648 -> true // Agility capes (alternate ids)
            617 -> true // Coins (alternate id)
            else -> false
        }
    }

    private fun writeNpcs(definitions: DefinitionSet, namer: EntityNamer) {
    ...
    private fun writeObjs(definitions: DefinitionSet, namer: EntityNamer) {
