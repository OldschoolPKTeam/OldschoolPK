...
import gg.rsmod.game.model.attr.RECENT_KILLS
...
object PlayerDeathAction {
    ...
    private suspend fun QueueTask.death(player: Player) {
        ...
        player.damageMap.getMostDamage()?.let { ref ->
            ref.get(player.world)?.let { killer ->
                if (killer is Player) {
                    world.getService(LoggerService::class.java, searchSubclasses = true)?.logPlayerKill(killer, player)
                    if(!killer.attr.has(RECENT_KILLS)) {
                        killer.attr[RECENT_KILLS] = EvictingQueue.create(3)
                    }
                    killer.attr[RECENT_KILLS]?.add(player.ipAddress)
                }
