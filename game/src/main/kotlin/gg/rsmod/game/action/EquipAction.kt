...
object EquipAction {
    ...
    fun hasPluginItemRequirements(p: Player, item: Item): Boolean {
        return p.world.plugins.executeEquipItemRequirement(p, item.id)
    }

    fun hasItemRequirements(p: Player, item: Item): Boolean {
    ...
    fun equip(p: Player, item: Item, inventorySlot: Int = -1): Result {
        ...
        if (!hasPluginItemRequirements(p, item)) {
            return Result.PLUGIN
        }

        if (!hasItemRequirements(p, item)) {
    ...
    /**
     * Unequip an item.
     * @param itemContainer The [ItemContainer] to deposit the item into.
     */
    fun unequip(p: Player, equipmentSlot: Int, itemContainer: ItemContainer = p.inventory): Result {
        p.attr[INTERACTING_ITEM_CONTAINER_KEY] = EQUIPMENT_KEY
        val item = p.equipment[equipmentSlot] ?: return Result.INVALID_ITEM

        val addition = itemContainer.add(item.id, item.amount, assureFullInsertion = false)
    ...
    /**
     * Execute actions that are taken when a player equips an item.
     */
    fun onItemEquip(p: Player, equipSlot: Int, itemId: Int) {
        p.resetFacePawn()
        p.world.plugins.executeEquipSlot(p, equipSlot)
        p.world.plugins.executeEquipItem(p, itemId)
    }

    /**
     * Execute actions that are taken when a player un-equips an item.
     */
