 ...
 */
class PlayerUpdateBlockSegment(val player: Player, val other: Player, private val newPlayer: Boolean) :
            ...
            UpdateBlockType.HITMARK -> {
                ...
                hits.forEach { hit ->
                    ...
                    for (i in 0 until hitmarks) {
                        ...
                        val hasTint = hitmark.isTintedHitType()
                        var shouldTint = 1 // 1 means true and will add tint to the hit mark
                        if(hasTint) {
                            if (player.index == other.index) {
                                shouldTint = 0
                            } else if (hitmark.attacker.isPresent) {
                                hitmark.attacker.get().get()?.let { attacker ->
                                    if (attacker.index == player.index)
                                        shouldTint = 0
                                }
                            }
                        }
                        buf.putSmart(hitmark.type + shouldTint)
