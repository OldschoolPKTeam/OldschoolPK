...
abstract class KotlinPlugin(private val r: PluginRepository, val world: World, val server: Server) {
    ...
    /**
     * Set this [Shape] to be a multi combat area.
     * @param heightLevel Is the height level. Use -1 (default) to set on all height levels.
     */
    fun set_multi_combat(heightLevel: Int = -1, shape: Shape) {
        if (heightLevel == -1) {
            repeat(4) { r.multiCombatAreas[it].add(shape) }
        } else {
            r.multiCombatAreas[0].add(shape)
        }
    }

    /**
     * Set this [Shape] to be a not multi combat area.
     * This is needed because some areas within multi-combat areas
     * are not multi-combat.
     * @param heightLevel Is the height level. Use -1 (default) to set on all height levels.
     */
    fun set_not_multi_combat(heightLevel: Int = -1, shape: Shape) {
        if (heightLevel == -1) {
            repeat(4) { r.notMultiCombatAreas[it].add(shape) }
        } else {
            r.notMultiCombatAreas[0].add(shape)
        }
    }
    ...
    /**
     * Spawn a [GroundItem] on the given coordinates that will never despawn.
     */
    fun spawn_world_item(item: Int, amount: Int, x: Int, z: Int, height: Int = 0, respawnCycles: Int = GroundItem.DEFAULT_RESPAWN_CYCLES) {
        val ground = GroundItem(item = item, amount = amount, tile = Tile(x, z, height), despawns = false)
        ground.respawnCycles = respawnCycles
        r.itemSpawns.add(ground)
    }

    /**
     * Spawn a [GroundItem] on the given coordinates that will despawn.
     */
    fun spawn_item(item: Int, amount: Int, x: Int, z: Int, height: Int = 0, respawnCycles: Int = GroundItem.DEFAULT_RESPAWN_CYCLES) {
        val ground = GroundItem(item = item, amount = amount, tile = Tile(x, z, height))
        ground.respawnCycles = respawnCycles
        r.itemSpawns.add(ground)
    }
    ...
    /**
     * Set the logic to execute when [gg.rsmod.game.message.impl.CloseModalMessage]
     * is handled on [interfaceId].
     */
    fun on_modal_close(interfaceId: Int, logic: (Plugin).() -> Unit) = r.bindModalClose(interfaceId, logic)
    ...
    /**
     * Invoked when a player attacks a pawn.
     */
    fun on_attack_pawn(plugin: Plugin.() -> Unit) = r.bindAttackPawn(plugin)
    ...
    /**
     * Invoked when npc finishes their death task and
     * is de-registered from the world.
     */
    fun on_npc_death(plugin: Plugin.() -> Unit) = r.bindGlobalNpcDeath(plugin)

    /**
     * Invoked when a [pawn] finishes their death task
     */
    fun on_pawn_death(plugin: Plugin.() -> Unit) = r.bindPawnDeath(plugin)
