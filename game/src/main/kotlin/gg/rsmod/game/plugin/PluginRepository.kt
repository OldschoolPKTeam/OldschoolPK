...
import gg.rsmod.game.model.attr.INTERACTING_ITEM_CONTAINER_KEY
...
class PluginRepository(val world: World) {
    ...
    /**
     * The plugins that will be executed when a defined modal is closed.
     */
    private val closeModalPlugins = Int2ObjectOpenHashMap<Plugin.() -> Unit>()
    ...
    /**
     * A list of plugins that will be invoked when a player attacks a pawn
     */
    private val attackPawnPlugins = mutableListOf<Plugin.() -> Unit>()
    ...
    /**
     * A list of plugins that will be invoked when a npc dies (after death animation played out).
     */
    private val globalNpcDeathPlugins = mutableListOf<Plugin.() -> Unit>()

    /**
     * A list of plugins that will be invoked when a pawn dies (after death animation played out).
     */
    private val pawnDeathPlugins = mutableListOf<Plugin.() -> Unit>()
    ...
    /**
     * Contains the multi-combat areas as a shape.
     */
    val multiCombatAreas = Array(4) { mutableListOf<Shape>() }

    /**
     * Contains the not multi-combat areas as a shape.
     * This exists because some areas on the map within multi-combat
     * areas are not multi.
     */
    val notMultiCombatAreas = Array(4) { mutableListOf<Shape>() }
    ...
    private fun scanPackageForPlugins(server: Server, world: World) {
        ClassGraph().enableAllInfo().whitelistModules().scan().use { result ->
            loadPlugins(server, world, result)
        }
    }
    ...
    private fun loadPlugins(server: Server, world: World, result: ScanResult) {
        val pluginExtensions = world.gameContext.pluginExtensions
        for (pluginExtension in pluginExtensions) {
            val scriptClass = result.getClassesWithAnnotation(KotlinScript::class.qualifiedName)
                    .map { Class.forName(it.name) }
                    .filter { pluginClass -> "$pluginExtension.kts" == pluginClass.getAnnotation(KotlinScript::class.java).fileExtension }
                    .singleOrNull { !it::class.isAbstract }
            check(scriptClass != null) { "No script class found with extension: $pluginExtension" }
            val plugins = result.getSubclasses(scriptClass.name).directOnly()
            plugins.forEach { p ->
                val pluginClass = p.loadClass(scriptClass)
                ...
                try {
                    ...
                    if (world.gameContext.assertScriptBinding) {
                        throw cause
                    } else {
                        logger.warn { "A script has caused an error to be thrown (assert-script-bind is false, so the error is ignored)" }
    ...
    fun bindAttackPawn(plugin: Plugin.() -> Unit) {
        attackPawnPlugins.add(plugin)
    }

    fun executeAttackPawn(p: Player) {
        attackPawnPlugins.forEach { plugin -> p.executePlugin(plugin) }
    }
    ...
    fun executePlayerDeath(p: Player) {
        ...
        pawnDeathPlugins.forEach { plugin -> p.executePlugin(plugin) }
    }
    ...
    fun executeNpcPreDeath(npc: Npc) {
        npcPreDeathPlugins[npc.id]?.let { plugin -> npc.executePlugin(plugin) }
    }
    ...
    fun executeNpcDeath(npc: Npc) {
        npcDeathPlugins[npc.id]?.let { plugin -> npc.executePlugin(plugin) }
        globalNpcDeathPlugins.forEach { plugin -> npc.executePlugin(plugin) }
        pawnDeathPlugins.forEach { plugin -> npc.executePlugin(plugin) }
    }

    fun bindGlobalNpcDeath(plugin: Plugin.() -> Unit) {
        globalNpcDeathPlugins.add(plugin)
    }

    fun bindPawnDeath(plugin: Plugin.() -> Unit) {
        pawnDeathPlugins.add(plugin)
    }
    ...
    fun bindModalClose(interfaceId: Int, plugin: Plugin.() -> Unit) {
        if (closeModalPlugins.containsKey(interfaceId)) {
            logger.error("Modal close is already bound to a plugin on $interfaceId")
            throw IllegalStateException("Modal close is already bound to a plugin on $interfaceId")
        }

        closeModalPlugins[interfaceId] = plugin
        pluginCount++
    }

    fun executeModalClose(p: Player, interfaceId: Int): Boolean {
        val plugin = closeModalPlugins[interfaceId] ?: return false
        p.executePlugin(plugin)
        return true
    }
    ...
    fun executeEquipmentOption(p: Player, item: Int, option: Int): Boolean {
        p.attr[INTERACTING_ITEM_CONTAINER_KEY] = EQUIPMENT_KEY
        val hash = (item shl 16) or option
    ...
    fun executeItem(p: Player, id: Int, opt: Int): Boolean {
        p.attr[INTERACTING_ITEM_CONTAINER_KEY] = INVENTORY_KEY
